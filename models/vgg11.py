...
class VGG11Encoder(nn.Module):
    # VGG11 backbone from the 2014 paper with BatchNorm added after each conv
    ...
    def forward(
        ...
        s4 = self.block4(s3)
        s5 = self.block5(s4)
        bottleneck = self.avgpool(s5)
        if return_features:
            return bottleneck, {"block1": s1, "block2": s2, "block3": s3, "block4": s4, "block5": s5}
        return bottleneck